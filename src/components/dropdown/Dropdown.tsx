  ...
  function Dropdown(
    ...
    useEffect(() => {
      if (refs.dropdown.current) {
        scrollIntoView(refs.dropdown.current);
      }
    ...
    const handleKeyDown = (e: KeyboardEvent): void => {
      if (isOpen) {
        ...
        const isCurrent = refs.field.current?.contains(target);
        ...
        // to navigate through the options
          ...
          if (matched.length && refs.dropdown.current) {
            ...
            if (target) {
              scrollToChild(refs.dropdown.current, target);
              target.focus();
            }
          }
    ...
    const onClickOutside = (event: MouseEvent): void => {
      if (!refs.field?.current?.parentNode?.contains(event.target as Node)) {
        onBlur && onBlur();
        toggleDropdown(false);
        setSearch("");
      }
